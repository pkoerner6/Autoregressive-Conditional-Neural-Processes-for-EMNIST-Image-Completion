...
def main(**kw_args):
    ...
    parser.add_argument("--root", type=str, nargs="*", default=["code/_experiments"])
    parser.add_argument("--subdir", type=str, nargs="*")
    parser.add_argument("--device", type=str, default="cuda" if torch.cuda.is_available() else "cpu")
    parser.add_argument("--gpu", type=int, default=0, help="GPU index to use (0 if one GPU)")
    parser.add_argument("--checkpoint-every", type=int, default=None)
    parser.add_argument("--dim-x", type=int, default=2) # for images we provide (x, y) pixel coordinates
    ...
    parser.add_argument("--evaluate-num-samples", type=int, default=512) # no impact for ConvCNP, because it is a deterministic model
    ...
    # ar_dir = "ar" if args.ar else "no_ar" # TODO
    # Setup script.
    if not observe:
